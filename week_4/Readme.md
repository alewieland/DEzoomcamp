...
CREATE OR REPLACE TABLE taxi-rides-ny-375620.trips_data_all.yellow_tripdata AS
...
FROM `taxi-rides-ny-375620.dbt_alwi.fact_trips`
WHERE
...
CREATE OR REPLACE EXTERNAL TABLE taxi-rides-ny-375620.trips_data_all.fhv_tripdata
  format = 'parquet',
  uris = ['gs://ny-taxi-trips-bocker/data/fhv/fhv_tripdata_*.parquet']
);
``` 

``` sql
-- Create table from external table
CREATE OR REPLACE TABLE taxi-rides-ny-375620.trips_data_all.fhv_tripdata AS
SELECT * FROM taxi-rides-ny-375620.trips_data_all.fhv_tripdata;
```

`dbt build --select stg_fhv_tripdata --var "is_test_run: false"`
``` sql
SELECT COUNT(pickup_datetime)
FROM `taxi-rides-ny-375620.dbt_alwi.stg_fhv_tripdata`
...
FROM `taxi-rides-ny-375620.dbt_alwi.fact_fhv_trips`
WHERE EXTRACT(YEAR FROM pickup_datetime) = 2019
GROUP BY month
ORDER BY month ASC
```
